...
public class MonopolyView extends javax.swing.JFrame {
    ...
    private int landOwnerships [];
    private LandsInfo[] landsInfo;
    private int turn;
    ...
    public MonopolyView() {
        ...
        this.addKeyListener(new KeyAdapter() {
            ...
            public void keyPressed(KeyEvent e) {
                ...
                //cheatCode = "UP UP DOWN DOWN LEFT LEFT RIGHT RIGHT A B A B";
                switch (e.getKeyCode()) {
                    case KeyEvent.VK_UP:
                        sequence += "UP ";
                        break;
    ...
    public void showMessage(String s){
        ...
        //String s = JOptionPane.showInputDialog("What you wanna type", "Enter Text");
    }
    ...
    public LandsInfo[] getLandsInfo() {
        return landsInfo;
    }
    
    public void setLandsInfo(LandsInfo[] landsInfo){
        this.landsInfo = landsInfo;
    }
    
    public void setInfos(Info info){
        this.infos = info;
        this.players = info.getPlayerStates();
        this.landOwnerships = info.getLandOwnerships();
        this.turn = info.getTurn();
    }
    
    public void update() {
        //get Info info from controller(from model) and show the players' position, balance and landOwnerShips
        setInfos(controller.update());
        ...
        for (int i = 1; i < landOwnerships.length; i++) {
            switch (landOwnerships[i]) {
                case 1:
                    if (p1Lands.length() > 0) {
                        p1Lands.append(", ");
                    }
        ...
        JLabel[] statuses = {p1StausStatueLabel, p2StausStatueLabel, p3StausStatueLabel, p4StausStatueLabel};
        for (int i = 0; i<4; i++){
            if (players[i].getIsBankrupt() == true)
                statuses[i].setText("Bankrupt");
            else
                statuses[i].setText("Active");
        }
        
        if (turn ==1){
            turnDisplayLabel.setText("Player 1");
        }else if (turn ==2){
            turnDisplayLabel.setText("Player 2");
        }else if (turn ==3){
            turnDisplayLabel.setText("Player 3");
        }else if (turn ==4){
            turnDisplayLabel.setText("Player 4");
        }
    }


    public void rollDice() {        
        //call the controller to handle roll dice functin and get a random int + show
        DiceDisplayLabel.setText(String.valueOf(controller.rollDice()));
    }
    
    public void updatePlayerPosition(){
        //call the controller to handle movement of player and calculate + show the new position
        controller.updatePlayerPosition(turnDisplayLabel.getText(),Integer.parseInt(DiceDisplayLabel.getText()));
    }
    
    public void passTurn(){
        //pass turn by changing the showing String
        controller.passTurn(turn);
        update();
    }
    
    public void loadLands(){
        //load slot/land info + show on form
        setLandsInfo(controller.loadLands());
    ...
    public void checkPositionToBeMoved(){
        ...
        switch(player){
            //check if the landing slot is not owned by anyone
            case"Player 1":
                //if on slot 1-23
                if(!p1PositionNumLabel.getText().equals("0")){
                    //load the ownership of that slot from model
                    setInfos(controller.update());
                    if(landOwnerships[Integer.parseInt(p1PositionNumLabel.getText())] == 0){
                        //if no one owned the slot, enable the buy/pass turn btn
                        buyLandBT.setEnabled(true);
                        passTurnBT.setEnabled(true);                        
                    }else{
                        if(landOwnerships[Integer.parseInt(p1PositionNumLabel.getText())] == 1){
                            //if land on his owned land -> enable pass turn btn
                            passTurnBT.setEnabled(true);
                        }else{
                            //if land on others' land -> enable pay rent btn
                            if(landOwnerships[Integer.parseInt(p1PositionNumLabel.getText())] != 1){
    ...
    public void buyLand() {
        //get slot current ownerships
        setInfos(controller.update());
        
        //get player
        String player = turnDisplayLabel.getText();
        
        //get current landOwneships & lands Info & players info from controller(from model)
        setLandsInfo(controller.loadLands());
        ...
        switch (player){
            case "Player 1":
                ...
                if(landPrice > players[0].getBalance()){
                    //no money, pass turn
                    showMessage("Notice: Not enough money to buy land");
                    buyLandBT.setEnabled(false);
                }else{
                    //controller handle buying land with slot no.+price
                    controller.buyLand(1, position, landPrice);                  
                }
                //buy land btn handler will then set itself enabled(false)
                break;
            case "Player 2":
    ...
    public void payRentalFee() {
        ...
        switch (player){
            case "Player 1":
                //get position , from String to int
                int position = Integer.parseInt(p1PositionNumLabel.getText());
                //get land price by slot no.
                int landPrice = landsInfo[position].getPrice();
                //call controller to handle pay rentalFee with payer, rentalFee and land owner
    ...
    public void editOnePlayerByNum(int choice, int player, int newValue) {
        // choice: 2 balance, 3 position, 4 status
        controller.editOnePlayerByNum(choice, player,newValue);
    }
    
    public void processingEdit(){
        int choice = askWhatChoice();
        
        //ask for choice 1-5, editing turn, balance, position, status or landOwnerships
        if (choice == 1){
            //edit turn
            int newTurn = askNewTurn();
            if (newTurn >=1 && newTurn <=4){
                controller.editTurn(newTurn);
                /*
                switch (newTurn){
                    case 1:
                        turn = 1;
                ...
                showMessage("Invalid input for turn");
            }
        } else if (choice >=2 && choice <=5){
            //choice 2-5
            //2 balance, 3 position, 4 status or 5 landOwnerships
            processPlayer(choice);   
        } else { //all invalid inputs for choice (including not int, >5, <1)
            showMessage("Invalid input for choice");
        }
    }
    
    public void processPlayer(int choice){
    //edit players' info
        //choice 2-5
        //2 balance, 3 position, 4 status or 5 landOwnerships
        int player = askWhichPlayer();
        if (player >=1 && player <= 4){
            if (choice != 5){
                //choice 2 3 4
                int newValue = askForNewValue(choice);
                if (newValue!= -1){
                    editOnePlayerByNum(choice, player, newValue);
                }else{
                    //processNewValue(choice,player);
                }
            }else{
                // choice =5, edit one player's landOwnership
                processNewLandOwnership(player);
            } 
        }else{ //all invalid inputs for player (including not int, >4, <1)
            showMessage("Invalid input for player");
            //processPlayer(choice);
        }
    }
    
    public void processNewValue(int choice, int player){
        int newValue = askForNewValue(choice);
        if (newValue != -1){
            switch(choice){
            case 2:
            ...
            case 4:
                if (newValue >1 || newValue < 0){
                    showMessage("Invalid input for status");
                    //processNewValue(choice,player);
                }else
                    controller.editOnePlayerByNum(choice, player, newValue);
                break;
    ...
    public void processNewLandOwnership(int player){
        try{
            String newLandOwnershipLine = askForNewLandOwnership();
            String newLandOwnershipString[] = newLandOwnershipLine.split(",");
            ArrayList<Integer> newLandOwnershipInt = new ArrayList();
            for (String s: newLandOwnershipString){
                newLandOwnershipInt.add(Integer.valueOf(s));
            }
            controller.editOnePlayerOwnership(player, newLandOwnershipInt);
        }
        catch(Exception e){
            showMessage("Invalid input for new landOwnership");
            //processNewLandOwnership(player);
        }
    }
    
    //asking inputs for edit
    public int askWhatChoice(){
        try{
            return Integer.parseInt(JOptionPane.showInputDialog(this, "Editing choice? "+"\n(1:Turn, 2:Balance, 3:Position, 4:Status , 5:LandOwnerships)", ""));
        }
        catch(Exception e){
            return -1;
        }
    ...
    public int askNewTurn(){
        try{
            return Integer.parseInt(JOptionPane.showInputDialog(this, "New Turn(1-4)? ", ""));
        }
        catch(Exception e){
            return -1;
        }
    ...
    public int askWhichPlayer(){
        try{
            return Integer.parseInt(JOptionPane.showInputDialog(this, "Editing Player(1-4)? ", ""));
        }
        catch(Exception e){
            return -1;
        }
    ...
    public int askForNewValue(int choice){
        //for choice 1-3, to get the int newValue
        int newValue = -1;
        try{
            switch(choice){
                case 2: // balance
                    newValue = Integer.parseInt(JOptionPane.showInputDialog(this, "New balance? ", ""));
                    break;
                case 3: // position
                    newValue = Integer.parseInt(JOptionPane.showInputDialog(this, "New Position (0-22)? "+"\nIMPORTANT: No passing-Go $2000 for editing", ""));
                    break;
                case 4: // status
                    newValue = Integer.parseInt(JOptionPane.showInputDialog(this, "New Status(1: Bankrupt, 0: Active)? ", ""));
                    break;
            }
            return newValue;
        }catch(Exception e){
    ...
    public String askForNewLandOwnership(){
        return JOptionPane.showInputDialog(this, "New ownerships(e.g. 1,2,5)? ", "");
    }
    ...
    private void initComponents() {
        ...
        slot1PanelLayout.setHorizontalGroup(
            ...
            .addGroup(slot1PanelLayout.createSequentialGroup()
                ...
                .addGroup(slot1PanelLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addComponent(slot1PLabel, javax.swing.GroupLayout.DEFAULT_SIZE, 96, Short.MAX_VALUE)))
        ...
        slot13PanelLayout.setHorizontalGroup(
            ...
            .addGroup(slot13PanelLayout.createSequentialGroup()
                ...
                .addGroup(slot13PanelLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addComponent(slot13PLabel, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))
                .addContainerGap())
        ...
        slot14PanelLayout.setHorizontalGroup(
            ...
            .addGroup(slot14PanelLayout.createSequentialGroup()
                ...
                .addGroup(slot14PanelLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addComponent(slot14PLabel, javax.swing.GroupLayout.DEFAULT_SIZE, 92, Short.MAX_VALUE))
                .addContainerGap())
        ...
        slot17PanelLayout.setHorizontalGroup(
            ...
            .addComponent(slot17PLabel, javax.swing.GroupLayout.DEFAULT_SIZE, 113, Short.MAX_VALUE)
        ...
        slot22PanelLayout.setVerticalGroup(
            slot22PanelLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, slot22PanelLayout.createSequentialGroup()
                .addGap(0, 0, Short.MAX_VALUE)
                .addGroup(slot22PanelLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(slot22PanelLayout.createSequentialGroup()
        ...
        editGameBT.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                editGameBTActionPerformed(evt);
            }
        });
            ...
            .addGroup(informationPanelLayout.createSequentialGroup()
                ...
                .addGroup(informationPanelLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(informationPanelLayout.createSequentialGroup()
                        ...
                        .addContainerGap())
                    .addGroup(informationPanelLayout.createSequentialGroup()
                        .addComponent(editGameBT, javax.swing.GroupLayout.DEFAULT_SIZE, 173, Short.MAX_VALUE)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
        ...
        informationPanelLayout.setVerticalGroup(
            ...
            .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, informationPanelLayout.createSequentialGroup()
                ...
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                .addComponent(NotificationLabel, javax.swing.GroupLayout.PREFERRED_SIZE, 62, javax.swing.GroupLayout.PREFERRED_SIZE)
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
        ...
        player1InfoPanelLayout.setHorizontalGroup(
            ...
            .addGroup(player1InfoPanelLayout.createSequentialGroup()
                .addGroup(player1InfoPanelLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            ...
            .addGroup(player1InfoPanelLayout.createSequentialGroup()
                ...
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                .addComponent(p1StausStatueLabel, javax.swing.GroupLayout.DEFAULT_SIZE, 52, Short.MAX_VALUE))
        );
            ...
            .addGroup(player2InfoPanelLayout.createSequentialGroup()
                .addGroup(player2InfoPanelLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(player2InfoPanelLayout.createSequentialGroup()
                        ...
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addComponent(p2StausStatueLabel, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))
                    .addGroup(player2InfoPanelLayout.createSequentialGroup()
                        .addGroup(player2InfoPanelLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            ...
            .addGroup(player3InfoPanelLayout.createSequentialGroup()
                .addGroup(player3InfoPanelLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(player3InfoPanelLayout.createSequentialGroup()
                        ...
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addComponent(p3StausStatueLabel, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))
                    .addGroup(player3InfoPanelLayout.createSequentialGroup()
                        .addGroup(player3InfoPanelLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            ...
            .addGroup(player4InfoPanelLayout.createSequentialGroup()
                .addGroup(player4InfoPanelLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(player4InfoPanelLayout.createSequentialGroup()
                        ...
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addComponent(p4StausStatueLabel, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))
                    .addGroup(player4InfoPanelLayout.createSequentialGroup()
                        .addGroup(player4InfoPanelLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            ...
            .addGroup(boardLayout.createSequentialGroup()
                ...
                .addGroup(boardLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(boardLayout.createSequentialGroup()
                        .addGroup(boardLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addGroup(boardLayout.createSequentialGroup()
                                .addComponent(slot5Panel, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                                .addGap(8, 8, 8))
                            .addComponent(slot12Panel, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))
                        .addGroup(boardLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                            .addGroup(boardLayout.createSequentialGroup()
                                .addComponent(slot4Panel, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(slot3Panel, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(emptySlotPanel0, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(slot2Panel, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(slot1Panel, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))
                            .addGroup(boardLayout.createSequentialGroup()
                                .addGap(4, 4, 4)
                                .addComponent(slot13Panel, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(slot14Panel, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(slot15Panel, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(slot16Panel, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
    ...
    private void editGameBTActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_editGameBTActionPerformed
        processingEdit();
        update();
    }//GEN-LAST:event_editGameBTActionPerformed
